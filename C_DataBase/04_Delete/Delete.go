...
func deleteUser(u User) (err error) {
	_, err = db.Exec("DELETE FROM updatetest WHERE Id=?", u.Id)
	return err
}
func main() {
	var (
	...
	db, err = sql.Open("mysql", "root:@/Learn")
	if err != nil {
		fmt.Println(err)
		panic(err)
	}
	...
	fmt.Println("下一步执行删除操作！")
	err = deleteUser(u)
	if err != nil {
		fmt.Println(err)
		panic(err)
	}
	fmt.Println("执行删除操作成功！")
	fmt.Println("下一步执行查找操作！")
	u, err = getOne(1)
	//此处应当报错no rows in result set
	if err != nil {
		fmt.Println(err)
		panic(err)
	}
